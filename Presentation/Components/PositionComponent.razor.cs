...
{
  public partial class PositionComponent : IDisposable
  {
    /// <summary>
    ...
    protected async Task CreateItems(IEnumerable<IAccountModel> accounts)
    {
      ...
      foreach (var position in positions)
      {
        ...
        item["PnL"] = string.Format("{0:0.00###}", position.GainLossAverageEstimate);
        item["Open Price"] = string.Format("{0:0.00###}", position.OpenPrice);
        item["Close Price"] = string.Format("{0:0.00###}", position.ClosePriceEstimate);
