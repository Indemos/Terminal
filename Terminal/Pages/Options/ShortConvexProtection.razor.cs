...
namespace Terminal.Pages.Options
{
  public partial class ShortConvexProtection
  {
    ...
    protected IList<OrderModel> GetOrders(DateTime? date) => Prime
      ...
      .Where(o => Equals(o?.Transaction?.Instrument?.Derivative?.TradeDate?.Date, date?.Date))
      .ToList();
    ...
    protected async Task OnTrade(PointModel point)
    {
      ...
      if (GetPositions(OptionDate(point)).Count == 4)
      {
        ...
        if (protectionDelta is 0 || isBuy || isSell)
        {
          var order = GetProtection(point, ProtectionOptions, isSell ? -1 : 1);

          await ClosePositions(o => Equals(o?.Transaction?.Instrument?.Derivative?.ExpirationDate?.Date, ProtectionDate(point).Date));
          await adapter.SendOrder(order);
        }
      }
    ...
    protected virtual (double, double, double) GetIndicators(PointModel point)
    {
      ...
      var options = GetPositions(OptionDate(point)).Where(o => o.Side is OrderSideEnum.Short);
      ...
      var positionSigma = options.Sum(o => o.Transaction.Instrument.Derivative.Volatility ?? 0);
    ...
    /// <returns></returns>
    protected OrderModel GetProtection(PointModel point, IList<InstrumentModel> options, int direction)
    {
      ...
      var order = new OrderModel
      {
        ...
        Transaction = new TransactionModel { Instrument = instrument }
      };
      ...
      var order = new OrderModel
      {
        ...
        [
          new OrderModel
          {
            ...
            Transaction = new TransactionModel { Instrument = longPut }
          },
          new OrderModel
          {
            Amount = 1,
            Side = OrderSideEnum.Long,
            Instruction = InstructionEnum.Side,
            Transaction = new TransactionModel { Instrument = longCall }
          },
          new OrderModel
          {
            Amount = 1,
            ...
            Instruction = InstructionEnum.Side,
            Transaction = new TransactionModel { Instrument = shortPut }
          },
          new OrderModel
          {
            Amount = 1,
            Side = OrderSideEnum.Short,
            Instruction = InstructionEnum.Side,
            Transaction = new TransactionModel { Instrument = shortCall }
          }
        ]
      };
      ...
      foreach (var position in adapter.Account.Positions.Values.ToList())
      {
        if (condition is null || condition(position))
        {
          var order = new OrderModel
          {
            ...
            Transaction = new TransactionModel { Instrument = position.Transaction.Instrument }
          };
